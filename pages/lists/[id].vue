    ...
    <div>
      ...
      <div class="w-full" v-if="items">
        ...
        <div
            class="divide-y divide-red-100"
        >
          <template v-for="element in boughtItems" :key="element.id">
            <div class="draggable flex py-2 items-center w-full">
              <span class="p-2">
              ...
              <input
                  ...
                  :value="element.name"
                  @change="updateItemName(element.id, $event.target.value)"
                  class="outline-none grow bg-transparent border-0 p-2 line-through"
              />
...
import { collection, doc, updateDoc, addDoc, orderBy, where, query, writeBatch, or, and, Timestamp } from 'firebase/firestore'
...
// is_bought = false
// OR
// is_bought = true AND updated_at = null OR updated_at <= now + 24h
...
const boughtItems = useCollection(query(
    itemsRef,
    and(
      where("is_bought", "==", true),
      where('updated_at', '>=', Timestamp.fromMillis(
